...
function parseHSL(hslStr: string): { h: number; s: number; l: number } {
  ...
  if (!match) {
    // Try matching space-separated without percent or comma-separated
    const altMatch = hslStr.match(/([\d.]+)[,\s]+([\d.]+)[%,\s]+([\d.]+)/);
    if (!altMatch) return { h: 220, s: 80, l: 60 };
    return { h: parseFloat(altMatch[1]), s: parseFloat(altMatch[2]), l: parseFloat(altMatch[3]) };
  }
